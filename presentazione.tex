...
\begin{frame}
	...
	\begin{center}
		\begin{large}
			Individuare automaticamente punti operazione per veicoli industriali a guida automatica (AGV)
		\end{large}
	\end{center}
	\begin{columns}
		\column{0.5\textwidth}
		\begin{itemize}
			\item Punto Operazione: Configurazione del veicolo che permette le operazioni di prelievo e deposito di pallet da scaffalature
			\item Dati di input: scansioni laser a larga scala
		\end{itemize}
		\column{0.5\textwidth}
		\begin{center}
			\vskip -0.5cm
...
\begin{frame}
	\frametitle{Metodi tradizionali di individuazione di Punti Operazione}
	\begin{columns}
		\column{0.6\textwidth}
			\begin{large}
				\begin{itemize}
					...
					\begin{itemize}
						...
						\item Difetti:
							\begin{itemize}
								\item Procedimento costoso (ore/uomo)
								\item Può richiedere il blocco della produzione
							\end{itemize}
					\end{itemize}
					\item Selezione manuale con il supporto di un visualizzatore 3D
					\begin{itemize}	%migliorare linguaggio
						%\item Meno complessa delle misurazioni sul campo
						\item Difetti:
							\begin{itemize}
								\item Più lenta di una selezione automatica
...
\begin{frame}
	...
	\begin{columns}
		\column{0.6\textwidth}
		\begin{itemize}
			\item Dati di input: scansioni laser 3D, acquisite con sensore Leica ScanStation P30
			...
			\item Ordine di grandezza: miliardi di punti per magazzini di decine di migliaia di metri quadrati
		\end{itemize}
		\column{0.4\textwidth}
		\begin{center}
			\vskip -1cm
	...
	\begin{columns}
		...
		Per convertire una Point Cloud in un'immagine 2D:
			\begin{enumerate}		%must be better
				\item Si assegna un valore ai punti 3D che stima il valore locale di curvatura
				\item Si genera una griglia 2D - ogni cella contiene la somma normalizzata dei valori dei punti 3D la cui proiezione appartiene alla cella
				\item Si assegna un colore in scala di grigi alle celle della griglia in base al valore
			\end{enumerate}
		\column{0.5\textwidth}
	...
	\begin{columns}
		\column{0.5\textwidth}
			\begin{itemize}
				...
				\item Il valore è determinato con una Cross Correlation (OpenCV) tra l'immagine e il template
...
\vskip -0.5cm
\begin{itemize}
	\item La linea blu rappresenta la fila di scaffali calcolata automaticamente
	\item Si può osservare un falso positivo all'estremo sinistro della fila
\end{itemize}
\vskip 0.5cm
\includegraphics[width=\textwidth]{Img/2D/found_curves_red_inv_arrow}
\end{frame}
...
\pgfplotsset{width = \textwidth}
	...
	\begin{columns}
		...
		\begin{itemize}
			...
			\item Correggere un falso positivo è preferibile a correggere un falso negativo
...
\begin{frame}
	...
	\begin{columns}
		\column{0.6\textwidth}
		\begin{itemize}
			\item La scelta del template può influenzare i risultati
			\item Il filtro NMS a volte elimina risultati corretti
			\item La funzione che stima le file di scaffali è inefficace se gli scaffali non sono in fila
		...
		\begin{center}
			\begin{large}
				\begin{itemize}
					...
					\vskip 0.5cm
					\item Integrazione del sistema di riconoscimento sviluppato nella fase di
					progettazione di magazzini automatici e verifica della precisione
				\end{itemize}
			\end{large}
		\end{center}
\end{frame}
